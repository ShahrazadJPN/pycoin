...
while True:
    ...
    if isOrdering is False and ewma1 > ewma5: #div > border and isOrdering is False and ewma1 > ewma3: # 25日平均に対する乖離率3%以上、上がり基調なので流れに乗って買う
        board = api.board(product_code="FX_BTC_JPY")
        price_now = board['mid_price']
        btc = money/price_now
    ...
    elif isOrdering is False and ewma1 < ewma5: # div < border * -1 and isOrdering is False and ewma1 < ewma3: # 同上、下げ基調 6時間移動平均が1日移動平均より↓
        board = api.board(product_code="FX_BTC_JPY")
        price_now = board['mid_price']
        btc = money/price_now
        #money = price_now * btc
